...
from net import build_model, parse_model_variant, ALL_MODEL_NAMES
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Benchmark all (or selected) model architectures."
    )
    parser.add_argument(
        "filters",
        nargs="*",
...
def main(argv=None):
    ...
    if args.filters:
        selected = [n for n in ALL_MODEL_NAMES
    ...
    try:
        ...
        if rows:
            ...
            print("-" * len(hdr))
            for r in rows:
                flops_disp = f"{r['flops_G']:.3f}" if isinstance(r['flops_G'], float) else r['flops_G']
                print(f"  {r['model']:<20} {r['in_channels']:>3}  "
